...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(phoneNumber => phoneNumber.split('').filter(char => /\d/.test(char)).join(''));
};
...
const allHappy = (arr) => {
  // Solution code here...
  let happy = true;
  arr.forEach(string => { if (!string.includes(':)')) happy = false; } );
  return happy;
};
...
const unenrollBrook = (arr) => {
  // Solution code here...
  return arr.map(classList => classList.filter( student => !student.includes('Brook') ));
};
