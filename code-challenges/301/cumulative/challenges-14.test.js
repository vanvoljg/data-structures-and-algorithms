...
let biggerThanLuke = (arr) => {
  ...
  let lukeMass = Number(arr.filter(character => /luke/i.test(character.name))[0].mass);

  return arr.reduce((charArray, character) => {
    if (Number(character.mass) > lukeMass) charArray.push(character.name);
    return charArray;
  }, []).join(' - ');
};
